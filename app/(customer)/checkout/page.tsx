...
export default function CheckoutPage() {
  ...
  // Trạng thái 3 bước: Xem đơn (review) -> Quét QR (qr) -> Thành công (success)
  ...
  // Khi khách hàng ấn "Tôi đồng ý" ở Popup Hợp đồng -> Chuyển sang màn hình QR
  const handleAcceptContract = () => {
    setCheckoutStep('qr');
    toast.success("Đã xác nhận Hợp đồng! Vui lòng quét mã QR thanh toán.", {
      style: { background: '#10b981', color: '#fff' }
    });
  };

  // Khi khách hàng ấn "Tôi đã chuyển khoản" -> Chuyển sang màn hình Thành công
  ...
  return (
    <div className="pt-24 pb-12 px-4 max-w-7xl mx-auto text-white min-h-screen">
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* BƯỚC 2: HIỂN THỊ MÃ QR THANH TOÁN (SAU KHI ĐỒNG Ý POPUP) */}
            {checkoutStep === 'qr' && (
      ...
      {/* Popup Pháp lý (Không cần sửa nếu bạn đã tách file riêng, nhưng để đảm bảo flow nó sẽ gọi ở bước 1) */}
      <LegalContractPopup 
        isOpen={isPopupOpen} 
